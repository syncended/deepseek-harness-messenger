window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULT_VOICE = Object.freeze({ enabled: true, model: "small", device: "auto" });
    const VOICE_MODELS = Object.freeze(["tiny", "base", "small", "medium", "large-v3", "turbo"]);
    const VOICE_DEVICES = Object.freeze(["auto", "cpu", "cuda"]);
    const CHAT_ID_PATTERN = /^-?\d+$/;
    const USER_ID_PATTERN = /^\d+$/;

    // Adapt DSH 0.1.2 controller remotes to the settings UI's response shape.
    // ConnectionHandle owns transport state only; it no longer exposes `.api`.
    function createSettingsApi(remote) {
      return {
        settings: {
          async describe() {
            return { result: await remote.settings.describe() };
          },
          async mutate({ ns, ops, expectedRevision }) {
            return { result: await remote.settings.mutate(ns, ops, expectedRevision) };
          },
        },
        credentials: {
          async describe({ refs }) {
            ...
            return { result: result.ok
              ? { ...result, value: { credentials: result.value } }
              : result };
          },
          async set({ ref, value }) {
    ...
    function voiceValue(section) {
      const voice = section?.voice;
      return {
        enabled: typeof voice?.enabled === "boolean" ? voice.enabled : DEFAULT_VOICE.enabled,
        model: VOICE_MODELS.includes(voice?.model) ? voice.model : DEFAULT_VOICE.model,
        device: VOICE_DEVICES.includes(voice?.device) ? voice.device : DEFAULT_VOICE.device,
      };
    }
    ...
    function validateVoice(form) {
      if (typeof form.enabled !== "boolean") throw new Error("Choose whether local voice transcription is enabled.");
      if (!VOICE_MODELS.includes(form.model)) throw new Error("Choose a supported Whisper model.");
      if (!VOICE_DEVICES.includes(form.device)) throw new Error("Choose auto, CPU, or CUDA for the voice device.");
      return { enabled: form.enabled, model: form.model, device: form.device };
    }
    ...
    function VoiceSettings({ scope }) {
      ...
      const save = async () => {
        ...
        if (saving.current || current.status !== "ready" || !current.writable) return;
        saving.current = true;
        setBusy(true);
        setError(null);
        setNotice(null);
        try {
          const next = validateVoice(form);
          await scope.set("voice", next);
          // Check the actual committed section, not fallback defaults: an older
          // Host that ignores voice must not produce a false success notice.
          if (!sameVoice(scope.getSnapshot().value?.voice, next)) {
            throw new Error("The Host rejected the voice settings. Reload and try again.");
          }
          setForm(next);
          setDirty(false);
          setNotice(next.enabled ? "Saved. Local voice transcription is enabled." : "Saved. Local voice transcription is disabled.");
        } catch (reason) {
      ...
      return h("div", { className: "dsh-msg-card" },
        h("div", { className: "dsh-msg-card-head" },
          h("div", null,
            h("h3", null, "Local Whisper"),
            h("p", null, "Local voice transcription on this DSH Host"),
          ),
        ),
        h(Toggle, {
        ...
        h("div", { className: "dsh-msg-grid" },
          h(Field, { label: "Whisper model", hint: "Larger models need more disk space and memory." },
            h("select", {
              className: "dsh-msg-input",
              value: form.model,
              disabled: !writable,
              onChange: (event) => update("model", event.target.value),
            }, ...VOICE_MODELS.map((model) => h("option", { key: model, value: model }, model))),
          ),
          h(Field, { label: "Voice device", hint: "Auto uses NVIDIA CUDA when available, otherwise CPU." },
            h("select", {
              className: "dsh-msg-input",
              value: form.device,
              disabled: !writable,
              onChange: (event) => update("device", event.target.value),
            }, ...VOICE_DEVICES.map((device) => h("option", { key: device, value: device }, device === "auto" ? "Auto" : device.toUpperCase()))),
          ),
        ),
        h("div", { className: "dsh-msg-hint" },
          h("p", null, "The first voice message lazily installs an isolated runtime and downloads the selected model. First preparation needs internet; saving these settings does not start a download."),
          h("p", null, "Audio is transcribed locally, not sent to a speech-to-text cloud. The model is kept on disk; the worker exits after the queue drains."),
          h("p", null, "Voice messages are limited to 5 minutes / 20 MB. Send ", h("code", null, "/voice_cancel"), " to cancel pending voice work."),
        ),
        ...
        h("div", { className: "dsh-msg-actions" },
          h("button", { type: "button", className: "dsh-msg-button dsh-msg-button-primary", disabled: !writable, onClick: save }, busy ? "Saving voice…" : "Save voice settings"),
          h("button", { type: "button", className: "dsh-msg-button", disabled: busy || !dirty, onClick: discard }, "Discard voice changes"),
        ),
      );
    }
    ...
    function MessengerSettings({ scope, settingsDescribe, api, credentialSignals }) {
      ...
      return h(
        ...
        h("div", { className: "dsh-msg-heading" },
          ...
          h("p", null, "Send ", h("code", null, "/notifications on"), " in an allowed Telegram chat to receive Host-wide automation statuses without selecting a session. Subscriptions survive restarts; ", h("code", null, "/notifications off"), " unsubscribes. Notifications include an Open session button and never switch sessions automatically."),
        ),
        h(
          "div",
    ...
    const inject = ["slots", "remote", "remote.settings", "remote.credentials", "settingsScope"];

    function apply(ctx) {
      const api = createSettingsApi(ctx.remote);
